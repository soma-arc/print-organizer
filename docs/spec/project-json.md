...
- ベイク結果は出力先の `report.json` に記録されるため、このファイルには含めない
...
└── output/
    ...
    ├── bricks.bin
    ├── mesh.stl
    └── report.json    ← genmesh が生成（実行統計・エラー。docs/schemas/report.v1.schema.json 参照）
```
...
```jsonc
{
    ...
    // genmesh (C++ CLI) 設定
    "genmesh": {
        "path":      "tools/genmesh/build/Release/genmesh.exe",  // 実行ファイルパス（省略時は自動探索）
...
| `mesh.iso` | `0.0` | |
| `mesh.adaptivity` | `0.0` | |
| `genmesh.path` | (自動探索) | 下記の探索順を参照 |
| `genmesh.write_vdb` | `false` | |
| `genmesh.skip` | `false` | |

### `genmesh.path` 探索順

`genmesh.path` が省略された場合、以下の優先順で実行ファイルを探索する：

| 優先度 | ソース | 用途 |
|--------|--------|------|
| 1 | `genmesh.path`（project.json） | 特殊な配置のプロジェクト |
| 2 | 環境変数 `PRINT_ORGANIZER_GENMESH` | 開発時（Debug/Release 切り替え） |
| 3 | GUI 実行ファイルの同梱ディレクトリ (`exe_dir/genmesh`) | 配布時 |
| 4 | PATH 探索 | 最後の手段 |
